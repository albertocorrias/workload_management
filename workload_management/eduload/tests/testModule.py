...
from eduload.models import Lecturer, Module, TeachingAssignment,WorkloadScenario, ModuleType, \
    Department,EmploymentTrack,ServiceRole, Faculty, Academicyear, ProgrammeOffered, UniversityStaff, TeachingAssignmentType
from eduload.forms import RemoveModuleForm
from eduload.global_constants import DEFAULT_WORKLOAD_NAME,DEFAULT_MODULE_TYPE_NAME

class TestModule(TenantTestCase):
    def setup_user(self):
    ...
    def test_add_remove_module_method(self):
        ...
        response = self.client.get(reverse('eduload:workloads_index'))
        ...
        response = self.client.get(reverse('eduload:scenario_view',  kwargs={'workloadscenario_id': new_scen.id}))
        self.assertEqual(response.status_code, 200) #No issues
        ...
        self.assertEqual(all_mods.filter(students_year_of_study__isnull=True).count(),0)#
        ...
        self.client.post(reverse('eduload:remove_module',  kwargs={'workloadscenario_id': new_scen.id}), {'select_module_to_remove': all_mods[0].id,'wipe_from_table' : RemoveModuleForm.REMOVE_COMPLETELY})
        response = self.client.get(reverse('eduload:scenario_view',  kwargs={'workloadscenario_id': new_scen.id}))
        self.assertEqual(response.status_code, 200) #No issues
    ...
    def test_add_remove_module_type(self):
        ...
        self.client.post(reverse('eduload:manage_module_type', kwargs={'department_id': first_dept.id}), {'type_name': 'TEST'})
        response = self.client.get(reverse('eduload:workloads_index'))
        self.assertEqual(response.status_code, 200) #No issues
        ...
        self.client.post(reverse('eduload:remove_module_type',kwargs={'department_id': first_dept.id}), {'select_module_type_to_remove': all_types[0].id})
        response = self.client.get(reverse('eduload:workloads_index'))
        self.assertEqual(response.status_code, 200) #No issues
        self.assertEqual(ModuleType.objects.all().count(),0)
    ...
    def test_add_remove_module_method_wipeout(self):
        ...
        self.client.post(reverse('eduload:add_assignment',  kwargs={'workloadscenario_id': new_scen.id}), data = {'select_lecturer': normal_lecturer.id, \
    ...
    def test_add_retire_module_method(self):
        ...
        self.client.post(reverse('eduload:remove_module',  kwargs={'workloadscenario_id': new_scen.id}), {'select_module_to_remove': all_mods[0].id,'wipe_from_table' : RemoveModuleForm.RETIRE_ONLY})
        response = self.client.get(reverse('eduload:scenario_view',  kwargs={'workloadscenario_id': new_scen.id}))
        self.assertEqual(response.status_code, 200) #No issues
    ...
    def test_add_module_same_code(self):
        ...
        mod_code = 'XXX1'
        #Add a new module
        self.client.post(reverse('eduload:add_module',  kwargs={'workloadscenario_id': new_scen.id}), {'module_code': mod_code, 'module_title' : 'testing', 'total_hours' : '234', 'module_type' : mod_type_1.id, 'students_year_of_study' :2, 'compulsory_in_primary_programme' : False, 'compulsory_in_secondary_programme':  False, 'compulsory_in_tertiary_programme': False,'semester_offered' : Module.UNASSIGNED,   'fresh_record' : True})
        response = self.client.get(reverse('eduload:scenario_view',  kwargs={'workloadscenario_id': new_scen.id}))
        self.assertEqual(response.status_code, 200) #No issues
        all_mods  = Module.objects.all();
        self.assertEqual(all_mods.count(),1)# mods now after this addition
        ...
        self.client.post(reverse('eduload:add_module',  kwargs={'workloadscenario_id': new_scen.id}), {'module_code': mod_code, 'module_title' : 'different', 'total_hours' : '234', 'module_type' : mod_type_1.id, 'semester_offered' : Module.UNASSIGNED,   'fresh_record' : True})    
        response = self.client.get(reverse('eduload:scenario_view',  kwargs={'workloadscenario_id': new_scen.id}))
        self.assertEqual(response.status_code, 200) #No issues
        ...
        self.client.post(reverse('eduload:manage_scenario'), {'label': scen_name_2, 'dept' : first_dept.id, 'copy_from': '', 'status': WorkloadScenario.DRAFT, 'fresh_record' : True, 'academic_year' : acad_year.id});
        scenario_2 = WorkloadScenario.objects.filter(label=scen_name_2)
        response = self.client.get(reverse('eduload:scenario_view',  kwargs={'workloadscenario_id': scenario_2.get().id}))
        self.assertEqual(response.status_code, 200) #No issues
        all_mods  = Module.objects.all();
        self.assertEqual(all_mods.count(),1)# mods now after this addition
        self.assertEqual(all_mods.filter(module_code=mod_code).exists(),True)
        ...
        self.client.post(reverse('eduload:add_module',  kwargs={'workloadscenario_id': scenario_2.get().id}), {'module_code': mod_code, 'module_title' : 'different', 'total_hours' : '234', 'module_type' : mod_type_1.id, 'students_year_of_study' :2, 'compulsory_in_primary_programme' : False, 'compulsory_in_secondary_programme':  False, 'compulsory_in_tertiary_programme': False,'semester_offered' : Module.UNASSIGNED,   'fresh_record' : True})    
        response = self.client.get(reverse('eduload:scenario_view',  kwargs={'workloadscenario_id': scenario_2.get().id}))
        self.assertEqual(response.status_code, 200) #No issues
    ...
    def test_add_module_with_no_hours(self):
        ...
        self.client.post(reverse('eduload:add_module',  kwargs={'workloadscenario_id': new_scen.id}), {'module_code': mod_code, 'module_title' : 'testing', 'module_type' : mod_type_1.id, 'students_year_of_study' :2, 'compulsory_in_primary_programme' : False, 'compulsory_in_secondary_programme':  False, 'compulsory_in_tertiary_programme': False, 'semester_offered' : Module.UNASSIGNED,   'fresh_record' : True})
        response = self.client.get(reverse('eduload:scenario_view',  kwargs={'workloadscenario_id': new_scen.id}))
        self.assertEqual(response.status_code, 200) #No issues
        all_mods  = Module.objects.all()
    #     self.assertEqual(all_mods.count(),1)# mods now after this addition
    ...
    def test_edit_existing_module(self):
        ...
        mod_code = 'XXX1'
        #Add a new module
        self.client.post(reverse('eduload:add_module',  kwargs={'workloadscenario_id': new_scen.id}), {
            'module_code': mod_code, 'module_title' : 'testing', 'total_hours' : '234', 
            'module_type' : mod_type_1.id, 'students_year_of_study' :2, 'compulsory_in_primary_programme' : False, 
            'compulsory_in_secondary_programme':  False, 'compulsory_in_tertiary_programme': False, 'semester_offered' : Module.UNASSIGNED,   'fresh_record' : True})
        response = self.client.get(reverse('eduload:scenario_view',  kwargs={'workloadscenario_id': new_scen.id}))
        self.assertEqual(response.status_code, 200) #No issues
        ...
        self.client.post(reverse('eduload:add_module',  kwargs={'workloadscenario_id': new_scen.id}), {'module_code': mod_code, 'module_title' : 'testing', 'total_hours' : '234', 'module_type' : mod_type_1.id, 'students_year_of_study' :2, 'compulsory_in_primary_programme' : False, 'compulsory_in_secondary_programme':  False, 'compulsory_in_tertiary_programme': False, 'semester_offered' : Module.UNASSIGNED,   'fresh_record' : False})    
        response = self.client.get(reverse('eduload:scenario_view',  kwargs={'workloadscenario_id': new_scen.id}))
        self.assertEqual(response.status_code, 200) #No issues        
        ...
        self.client.post(reverse('eduload:add_module',  kwargs={'workloadscenario_id': new_scen.id}), {'module_code': mod_code, 'module_title' : 'testing', 'total_hours' : '10', 'module_type' : mod_type_1.id, 'students_year_of_study' :2, 'compulsory_in_primary_programme' : False, 'compulsory_in_secondary_programme':  False, 'compulsory_in_tertiary_programme': False, 'semester_offered' : Module.UNASSIGNED,   'fresh_record' : False})    
        response = self.client.get(reverse('eduload:scenario_view',  kwargs={'workloadscenario_id': new_scen.id}))
        self.assertEqual(response.status_code, 200) #No issues
        all_mods = Module.objects.all()
        ...
        self.client.post(reverse('eduload:add_module',  kwargs={'workloadscenario_id': new_scen.id}), {'module_code': mod_code, \
        'module_title' : 'testing', 'total_hours' : '10', 'module_type' : mod_type_1.id, 'compulsory_in_primary_programme' : Module.YES,\
        'students_year_of_study' : 2,  'compulsory_in_secondary_programme':  False, 'compulsory_in_tertiary_programme': False, 
        ...
        self.client.post(reverse('eduload:add_module',  kwargs={'workloadscenario_id': new_scen.id}), {'module_code': mod_code, \
        'module_title' : 'hello', 'total_hours' : '10', 'module_type' : mod_type_1.id, 'compulsory_in_primary_programme' : Module.YES,\
        'students_year_of_study' : 2,  'compulsory_in_secondary_programme':  False, 'compulsory_in_tertiary_programme': False, 
        'semester_offered' : Module.UNASSIGNED,   'fresh_record' : False})    
        response = self.client.get(reverse('eduload:scenario_view',  kwargs={'workloadscenario_id': new_scen.id}))
        self.assertEqual(response.status_code, 200) #No issues
        ...
        'module_title' : 'hello', 'total_hours' : '10', 'module_type' : mod_type_1.id, 'compulsory_in_primary_programme' : Module.YES,\
        'students_year_of_study' : 2,  'compulsory_in_secondary_programme':  False, 'compulsory_in_tertiary_programme': False, 
        'semester_offered' : Module.UNASSIGNED,   'primary_programme' : prog_1.id, 'fresh_record' : False})
        ...
        'module_title' : 'hello', 'total_hours' : '10', 'module_type' : mod_type_1.id, 'compulsory_in_primary_programme' : Module.YES,\
        'students_year_of_study' : 2,  'compulsory_in_secondary_programme':  False, 'compulsory_in_tertiary_programme': False, 
        'semester_offered' : Module.UNASSIGNED,   'primary_programme' : prog_1.id, 'secondary_programme' : prog_2.id, 'fresh_record' : False})
        ...
        'module_title' : 'hello', 'total_hours' : '10', 'module_type' : mod_type_1.id, 'compulsory_in_primary_programme' : Module.YES,\
        'students_year_of_study' : 2,  'compulsory_in_secondary_programme':  False, 'compulsory_in_tertiary_programme': False, 
        'semester_offered' : Module.UNASSIGNED,   'primary_programme' : prog_1.id, 'secondary_programme' : prog_2.id, 'tertiary_programme' : prog_3.id, 'fresh_record' : False})
        ...
        self.assertEqual(all_mods.filter(compulsory_in_secondary_programme=True).count(),0)
        self.assertEqual(all_mods.filter(compulsory_in_tertiary_programme=True).count(),0)
        #Now edit.MAke it compulsory in both econdary and tertiary programmes
        self.client.post(reverse('eduload:add_module',  kwargs={'workloadscenario_id': new_scen.id}), \
        {'module_code': mod_code, \
        'module_title' : 'hello', 'total_hours' : '10', 'module_type' : mod_type_1.id, 'compulsory_in_primary_programme' : Module.YES,\
        'students_year_of_study' : 2,  'compulsory_in_secondary_programme':  True, 'compulsory_in_tertiary_programme': True, 
        'semester_offered' : Module.UNASSIGNED,   'primary_programme' : prog_1.id, 'secondary_programme' : prog_2.id, 'tertiary_programme' : prog_3.id, 'fresh_record' : False})
    ...
    def test_edit_existing_module_with_multiple_scenarios(self):
        ...
        response = self.client.get(reverse('eduload:scenario_view',  kwargs={'workloadscenario_id': first_scen.id}))
        self.assertEqual(response.status_code, 200) #No issues
        ...
        mod_code = 'XXX1'
        #Add a new module
        self.client.post(reverse('eduload:add_module',  kwargs={'workloadscenario_id': first_scen.id}), {'module_code': mod_code,\
         'module_title' : 'testing', 'total_hours' : '234', 'module_type' : mod_type_1.id, 'semester_offered' : Module.UNASSIGNED, \
         'compulsory_in_primary_programme' : False,'compulsory_in_secondary_programme':  False, 'compulsory_in_tertiary_programme': False,'students_year_of_study' : 1,
          'fresh_record' : True})
        response = self.client.get(reverse('eduload:scenario_view',  kwargs={'workloadscenario_id': first_scen.id}))
        all_mods = Module.objects.all()
        self.assertEqual(response.status_code, 200) #No issues
        ...
        new_label = 'new_scen'
        acad_year = Academicyear.objects.create(start_year=2200)
        self.client.post(reverse('eduload:manage_scenario'), {'label': new_label, 'dept' : first_dept.id, 'copy_from':default_scen_id, 'status': WorkloadScenario.DRAFT, 'fresh_record' : True, 'academic_year' :  acad_year.id});
        new_scenario = WorkloadScenario.objects.filter(label=new_label)
        #Activate second scenario
        response = self.client.get(reverse('eduload:scenario_view',  kwargs={'workloadscenario_id': new_scenario.get().id}))
        ...
        #Now edit the existing module. KEY CHANGHE: total hours from 234 to 10
        self.client.post(reverse('eduload:add_module',  kwargs={'workloadscenario_id': new_scenario.get().id}), {'module_code': mod_code,\
         'module_title' : 'testing', 'total_hours' : '10', 'module_type' : mod_type_1.id, 'semester_offered' : Module.UNASSIGNED, \
         'compulsory_in_primary_programme' : False,'compulsory_in_secondary_programme':  False, 'compulsory_in_tertiary_programme': False,'students_year_of_study' : 1,
          'fresh_record' : False})
    ...
    def test_remove_module_with_assignments_multiple_scens(self):
        ...
        self.client.post(reverse('eduload:add_professor',  kwargs={'workloadscenario_id': scenario_2.id}),{'name':'normal_lecturer','fraction_appointment' : '0.7',    'service_role' : new_role.id, 'employment_track': new_track.id, 'is_external': False,'fresh_record' : True})
        self.client.post(reverse('eduload:add_professor',  kwargs={'workloadscenario_id': scenario_2.id}),{'name':'educator_track','fraction_appointment' : '1.0',     'service_role' : new_role.id,  'employment_track': new_track.id, 'is_external': False,'fresh_record' : True})
        self.client.post(reverse('eduload:add_professor',  kwargs={'workloadscenario_id': scenario_2.id}),{'name':'vice_dean','fraction_appointment' : '0.5',     'service_role' : new_role.id, 'employment_track': new_track.id, 'is_external': False,'fresh_record' : True})
        ...
        mod_code_2 = 'AS201'
        mod_code_3 = 'AS301'
        self.client.post(reverse('eduload:add_module',  kwargs={'workloadscenario_id': scenario_2.id}), {'module_code': mod_code_1, 'module_title' : 'module 1', 'total_hours' : '234', 'module_type' : mod_type_1.id, 'students_year_of_study' :2, 'compulsory_in_primary_programme' : False, 'compulsory_in_secondary_programme':  False, 'compulsory_in_tertiary_programme': False,'semester_offered' : Module.UNASSIGNED,   'fresh_record' : True})    
        self.client.post(reverse('eduload:add_module',  kwargs={'workloadscenario_id': scenario_2.id}), {'module_code': mod_code_2, 'module_title' : 'module 2', 'total_hours' : '234', 'module_type' : mod_type_1.id, 'students_year_of_study' :2, 'compulsory_in_primary_programme' : False, 'compulsory_in_secondary_programme':  False, 'compulsory_in_tertiary_programme': False,'semester_offered' : Module.UNASSIGNED,   'fresh_record' : True})    
        self.client.post(reverse('eduload:add_module',  kwargs={'workloadscenario_id': scenario_2.id}), {'module_code': mod_code_3, 'module_title' : 'module 3', 'total_hours' : '234', 'module_type' : mod_type_1.id, 'students_year_of_study' :2, 'compulsory_in_primary_programme' : False, 'compulsory_in_secondary_programme':  False, 'compulsory_in_tertiary_programme': False,'semester_offered' : Module.UNASSIGNED,   'fresh_record' : True})    
        ...
        self.client.post(reverse('eduload:add_assignment',  kwargs={'workloadscenario_id': scenario_2.id}), data = {'select_lecturer': educator_track.id, \
                 'select_module': module_2.id, 'teaching_assignment_type' : assignment_type.id, 'counted_towards_workload' : 'yes','how_many_units':'56'})
                
        response = self.client.get(reverse('eduload:scenario_view',  kwargs={'workloadscenario_id': scenario_2.id}))
        self.assertEqual(response.status_code, 200) #No issues
        ...
        self.client.post(reverse('eduload:remove_module',  kwargs={'workloadscenario_id': scenario_2.id}), {'select_module_to_remove': module_2.id, 'wipe_from_table' : RemoveModuleForm.RETIRE_ONLY})
        #Check after
        response = self.client.get(reverse('eduload:scenario_view',  kwargs={'workloadscenario_id': scenario_2.id}))
        self.assertEqual(response.status_code, 200) #No issues
        all_mods = Module.objects.all()
        ...
        self.client.post(reverse('eduload:add_assignment',  kwargs={'workloadscenario_id': scenario_2.id}), data = {'select_lecturer': normal_lecturer.id, \
                 'select_module': module_2.id, 'teaching_assignment_type' : assignment_type.id, 'counted_towards_workload' : 'yes','how_many_units':'56'})
        ...
        response = self.client.get(reverse('eduload:scenario_view',  kwargs={'workloadscenario_id': scenario_1.id}))

        #Re-add smae people, same modules to scenario 1
        self.client.post(reverse('eduload:add_professor',  kwargs={'workloadscenario_id': scenario_1.id}),{'name':'normal_lecturer','fraction_appointment' : '0.7',     'service_role' : new_role.id, 'employment_track': new_track.id, 'is_external': False,'fresh_record' : True})
        self.client.post(reverse('eduload:add_professor',  kwargs={'workloadscenario_id': scenario_1.id}),{'name':'educator_track','fraction_appointment' : '1.0',     'service_role' : new_role.id, 'employment_track': new_track.id, 'is_external': False,'fresh_record' : True})
        self.client.post(reverse('eduload:add_professor',  kwargs={'workloadscenario_id': scenario_1.id}),{'name':'vice_dean','fraction_appointment' : '0.5',     'service_role' : new_role.id, 'employment_track': new_track.id, 'is_external': False,'fresh_record' : True})
        ...
        self.client.post(reverse('eduload:add_module',  kwargs={'workloadscenario_id': scenario_1.id}), {'module_code': mod_code_1, 'module_title' : 'module 1', 'total_hours' : '234', 'module_type' : mod_type_1.id, 'students_year_of_study' :2, 'compulsory_in_primary_programme' : False, 'compulsory_in_secondary_programme':  False, 'compulsory_in_tertiary_programme': False,'semester_offered' : Module.UNASSIGNED,   'fresh_record' : True})    
        self.client.post(reverse('eduload:add_module',  kwargs={'workloadscenario_id': scenario_1.id}), {'module_code': 'M2SC1', 'module_title' : 'module 2 scenario 1', 'total_hours' : '234', 'module_type' : mod_type_1.id, 'students_year_of_study' :2, 'compulsory_in_primary_programme' : False, 'compulsory_in_secondary_programme':  False, 'compulsory_in_tertiary_programme': False, 'semester_offered' : Module.UNASSIGNED,   'fresh_record' : True})    
        self.client.post(reverse('eduload:add_module',  kwargs={'workloadscenario_id': scenario_1.id}), {'module_code': mod_code_3, 'module_title' : 'module 3', 'total_hours' : '234', 'module_type' : mod_type_1.id, 'students_year_of_study' :2, 'compulsory_in_primary_programme' : False, 'compulsory_in_secondary_programme':  False, 'compulsory_in_tertiary_programme': False,'semester_offered' : Module.UNASSIGNED,   'fresh_record' : True})    
        ...
        module_2_scen_1 = Module.objects.filter(module_code = 'M2SC1').filter(scenario_ref__label = scen_name_1).get()
        ...
        self.client.post(reverse('eduload:add_assignment',  kwargs={'workloadscenario_id': scenario_1.id}), data = {'select_lecturer': normal_lecturer_scen_1.id, \
        ...
        self.client.post(reverse('eduload:add_assignment',  kwargs={'workloadscenario_id': scenario_1.id}), data = {'select_lecturer': vice_dean_scen_1.id, \
        ...
        self.client.post(reverse('eduload:remove_module',  kwargs={'workloadscenario_id': scenario_1.id}), {'select_module_to_remove': module_2_scen_1.id, 'wipe_from_table' : RemoveModuleForm.RETIRE_ONLY})
        response = self.client.get(reverse('eduload:scenario_view',  kwargs={'workloadscenario_id': scenario_1.id}))
        self.assertEqual(response.status_code, 200) #No issues
        all_mods = Module.objects.all()
        ...
        #Now wipe module 2
        self.client.post(reverse('eduload:remove_module',  kwargs={'workloadscenario_id': scenario_2.id}), {'select_module_to_remove': module_2.id, 'wipe_from_table' : RemoveModuleForm.REMOVE_COMPLETELY})
        response = self.client.get(reverse('eduload:scenario_view',  kwargs={'workloadscenario_id': scenario_2.id}))
        self.assertEqual(response.status_code, 200) #No issues
        all_mods = Module.objects.all()
    ...
    def test_remove_module_type_with_modules_and_assignments(self):
        ...
        mod_code_3 = 'AS301'
        self.client.post(reverse('eduload:add_module',  kwargs={'workloadscenario_id': scenario_2.id}), {'module_code': mod_code_1, 'module_title' : 'module 1', 'total_hours' : '234', 'module_type' : mod_type_1.id, 'students_year_of_study' :2, 'compulsory_in_primary_programme' : False, 'compulsory_in_secondary_programme':  False, 'compulsory_in_tertiary_programme': False,'semester_offered' : Module.UNASSIGNED,   'fresh_record' : True})    
        self.client.post(reverse('eduload:add_module',  kwargs={'workloadscenario_id': scenario_2.id}), {'module_code': mod_code_2, 'module_title' : 'module 2', 'total_hours' : '234', 'module_type' : mod_type_2.id, 'students_year_of_study' :2, 'compulsory_in_primary_programme' : False, 'compulsory_in_secondary_programme':  False, 'compulsory_in_tertiary_programme': False,'semester_offered' : Module.UNASSIGNED,   'fresh_record' : True})    
        self.client.post(reverse('eduload:add_module',  kwargs={'workloadscenario_id': scenario_2.id}), {'module_code': mod_code_3, 'module_title' : 'module 3', 'total_hours' : '234', 'module_type' : mod_type_1.id, 'students_year_of_study' :2, 'compulsory_in_primary_programme' : False, 'compulsory_in_secondary_programme':  False, 'compulsory_in_tertiary_programme': False,'semester_offered' : Module.UNASSIGNED,   'fresh_record' : True})    
        self.assertEqual(Module.objects.all().count(),3)
        ...
        self.client.post(reverse('eduload:add_module',  kwargs={'workloadscenario_id': scenario_1.id}), {'module_code': mod_code_1, 'module_title' : 'module 1', 'total_hours' : '234', 'module_type' : mod_type_1.id, 'students_year_of_study' :2, 'compulsory_in_primary_programme' : False, 'compulsory_in_secondary_programme':  False, 'compulsory_in_tertiary_programme': False, 'semester_offered' : Module.UNASSIGNED,   'fresh_record' : True})    
        self.client.post(reverse('eduload:add_module',  kwargs={'workloadscenario_id': scenario_1.id}), {'module_code': mod_code_2, 'module_title' : 'module 2 scenario 1', 'total_hours' : '234', 'module_type' : mod_type_2.id, 'students_year_of_study' :2, 'compulsory_in_primary_programme' : False, 'compulsory_in_secondary_programme':  False, 'compulsory_in_tertiary_programme': False,'semester_offered' : Module.UNASSIGNED,   'fresh_record' : True})    
        self.client.post(reverse('eduload:add_module',  kwargs={'workloadscenario_id': scenario_1.id}), {'module_code': mod_code_3, 'module_title' : 'module 3', 'total_hours' : '234', 'module_type' : mod_type_1.id, 'students_year_of_study' :2, 'compulsory_in_primary_programme' : False, 'compulsory_in_secondary_programme':  False, 'compulsory_in_tertiary_programme': False,'semester_offered' : Module.UNASSIGNED,   'fresh_record' : True})    
        self.assertEqual(Module.objects.all().count(),6)#3+3
        ...
        self.client.post(reverse('eduload:remove_module_type', kwargs={'department_id': first_dept.id}), {'select_module_type_to_remove': mod_type_2.id})
    ...
    def test_add_modules_in_bulk(self):
        ...
        response = self.client.post(reverse('eduload:bulk_add_module',  kwargs={'workload_id': scenario_1.id}),\
