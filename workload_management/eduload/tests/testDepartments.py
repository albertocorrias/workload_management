...
class TestDepartments(TenantTestCase):
    def setup_user(self):
    ...
    def test_add_remove(self):
        ...
        new_dept_code = 'NN'
        #add  one
        self.client.post(reverse('eduload:manage_department'),{'department_name':new_dept_name, 'department_acronym' : new_dept_code,\
        ...
        #Remove department
        self.client.post(reverse('eduload:remove_department'),{'select_department_to_remove':new_dept_object.get().id})
        self.assertEqual(Department.objects.all().count(),0) #
    ...
    def test_add_remove_with_workloads(self):
        ...
        self.client.post(reverse('eduload:manage_scenario'), {'label': 'test_scen', 'dept' : new_dept_object.get().id, 'status': WorkloadScenario.DRAFT, 'copy_from':'','fresh_record' : True, 'academic_year' : acad_year.id});
    ...
    def test_edit_department(self):
        ...
        dept_name = 'my_dept'
        dept_code = 'NN'
        self.client.post(reverse('eduload:manage_department'),{'department_name':dept_name, 'department_acronym' : dept_code,\
                        'faculty' : new_fac.id, 'fresh_record' :  True})
        self.assertEqual(Department.objects.all().count(),1) 
        ...
        new_name = "new name"
        self.client.post(reverse('eduload:manage_department'),{'department_name':new_name, 'department_acronym' : dept_code,\
        ...
        self.client.post(reverse('eduload:manage_department'),{'department_name':new_name, 'department_acronym' : new_acronym,\
                        'faculty' : new_fac.id, 'fresh_record' :  False,
                        'dept_id' : dept_id})
        self.assertEqual(Department.objects.all().count(),1) #still 1
        self.assertEqual(Department.objects.filter(department_name = dept_name).count(), 0) #should be gone
        ...
        new_fac_code = 'NN'
        #add another one
        self.client.post(reverse('eduload:manage_faculty'),{'faculty_name':new_fac_name, 'faculty_acronym' : new_fac_code, 'fresh_record' :  True})
    ...
    def test_edit_department_into_same_name(self):
        ...
        dept_name_1= 'dept_1'
        dept_code_1 = 'NN1'
        self.client.post(reverse('eduload:manage_department'),{'department_name':dept_name_1, 'department_acronym' : dept_code_1,\
                    'faculty' : new_fac.id, 'fresh_record' :  True})

        dept_name_2= 'dept_2'
        dept_code_2 = 'NN2'
        self.client.post(reverse('eduload:manage_department'),{'department_name':dept_name_2, 'department_acronym' : dept_code_2,\
                    'faculty' : new_fac.id, 'fresh_record' :  True})
        ...
        self.client.post(reverse('eduload:manage_department'),{'department_name':dept_name_2, 'department_acronym' : dept_code_1,\
                    'faculty' : new_fac.id, 'fresh_record' :  False,
                    'dept_id' : dept_id})
    ...
    def testDepartmentSummaryViewAndAddNewScenario(self):
        ...
        response = self.client.get(reverse('eduload:department', kwargs={'department_id': test_dept.id}))
        ...
        response = self.client.post(reverse('eduload:department', kwargs={'department_id': test_dept.id}), {"select_academic_year" : test_acad_year.id})
        ...
        self.client.post(reverse('eduload:department', kwargs={'department_id': test_dept.id}),{\
                        'label':new_label, \
                        'dept' : test_dept.id,\
        ...
        self.client.post(reverse('eduload:department', kwargs={'department_id': test_dept.id}), {'select_scenario_to_remove': new_scen.id})
