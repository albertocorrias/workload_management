...
from decimal import *
from eduload.models import Lecturer, Module, TeachingAssignment,WorkloadScenario,\
...
from eduload.global_constants import MAX_NUMBER_OF_CHARACTERS_IN_TABLE_CELL, ShortenString,\
    CalculateNumHoursBasedOnWeeklyInfo,csv_file_type,requested_table_type
from eduload.helper_methods import RegularizeName,CalculateEmploymentTracksTable,CalculateServiceRolesTable,\
...
class testHelperMethods(TenantTestCase):
    ...
    def test_workload_tables(self):
        ...
        normal_lecturer = Lecturer.objects.create(name="normal_lecturer", fraction_appointment = 0.7,employment_track=track_def, workload_scenario=first_scen, service_role=srvc_role)
        educator_track = Lecturer.objects.create(name="educator_track", fraction_appointment = 1.0, employment_track=track_1, workload_scenario=first_scen, service_role=srvc_role)
        vice_dean = Lecturer.objects.create(name="vice dean", fraction_appointment = 0.5  , employment_track=track_2, workload_scenario=first_scen, service_role=srvc_role)
        ...
        self.client.post(reverse('eduload:add_module',  kwargs={'workloadscenario_id': first_scen.id}), {'module_code': mod_code_1, 'module_title' : 'module 1', 'total_hours' : '52', 'module_type' : mod_type_1.id, 'students_year_of_study' :2, 'compulsory_in_primary_programme' : False, 'compulsory_in_secondary_programme':  False, 'compulsory_in_tertiary_programme': False,'semester_offered' : Module.UNASSIGNED,   'primary_programme' : prog.id, 'fresh_record' : True})    
        self.client.post(reverse('eduload:add_module',  kwargs={'workloadscenario_id': first_scen.id}), {'module_code': mod_code_2, 'module_title' : 'module 2', 'total_hours' : '52', 'module_type' : mod_type_1.id, 'students_year_of_study' :2, 'compulsory_in_primary_programme' : False, 'compulsory_in_secondary_programme':  False, 'compulsory_in_tertiary_programme': False,'semester_offered' : Module.UNASSIGNED,   'primary_programme' : prog.id, 'fresh_record' : True})    
        self.client.post(reverse('eduload:add_module',  kwargs={'workloadscenario_id': first_scen.id}), {'module_code': mod_code_3, 'module_title' : 'module 3', 'total_hours' : '52', 'module_type' : mod_type_1.id, 'students_year_of_study' :2, 'compulsory_in_primary_programme' : False, 'compulsory_in_secondary_programme':  False, 'compulsory_in_tertiary_programme': False,'semester_offered' : Module.UNASSIGNED,   'primary_programme' : prog.id, 'fresh_record' : True})    
        ...
        self.assertAlmostEqual(float(obtained_wl_table[0]["prof_balance"]), float(obtained_wl_table[0]["total_hours_for_prof"]) - float(obtained_wl_table[0]["prof_expected_hours"]))
        ...
        self.assertAlmostEqual(float(obtained_wl_table[1]["prof_balance"]), float(obtained_wl_table[1]["total_hours_for_prof"]) - float(obtained_wl_table[1]["prof_expected_hours"]))
        ...
        self.assertAlmostEqual(float(obtained_wl_table[2]["prof_balance"]), float(obtained_wl_table[2]["total_hours_for_prof"]) - float(obtained_wl_table[2]["prof_expected_hours"]))
    ...
    def test_workload_summary_method(self):
        ...
        track_2 = EmploymentTrack.objects.create(track_name = "track_2", track_adjustment = 3.0, is_adjunct = True, faculty=first_fac)

        normal_lecturer = Lecturer.objects.create(name="normal_lecturer", fraction_appointment = 0.7, employment_track=track_def, service_role = srvc_role, workload_scenario = first_scen )
        educator_track = Lecturer.objects.create(name="educator_track", fraction_appointment = 1.0, employment_track=track_1, service_role = srvc_role, workload_scenario = first_scen)
        vice_dean = Lecturer.objects.create(name="vice_dean", fraction_appointment = 0.5, employment_track = track_def, service_role = srvc_role, workload_scenario = first_scen) #should use default is_adjunct = false)
        ...
        self.assertEqual(vice_dean.employment_track.track_name,"track default")

        adjunct_lecturer = Lecturer.objects.create(name="adjunct_lecturer", fraction_appointment = 0.5, employment_track=track_2, service_role = srvc_role, workload_scenario = first_scen)
        external_lecturer = Lecturer.objects.create(name="external", fraction_appointment = 0.8, employment_track = track_1, service_role = srvc_role, workload_scenario = first_scen, is_external = True)
        ...
        self.client.post(reverse('eduload:add_module',  kwargs={'workloadscenario_id': first_scen.id}), \
        ...
        self.client.post(reverse('eduload:add_assignment', kwargs={'workloadscenario_id': first_scen.id}), {'select_lecturer': normal_lecturer.id, 'select_module': module_1.id,'teaching_assignment_type' : assignment_type.id, 'counted_towards_workload' : 'yes','how_many_units':'36'})
        self.client.post(reverse('eduload:add_assignment', kwargs={'workloadscenario_id': first_scen.id}), {'select_lecturer': vice_dean.id, 'select_module': module_1.id, 'teaching_assignment_type' : assignment_type.id, 'counted_towards_workload' : 'yes','how_many_units':'6'})
        self.client.post(reverse('eduload:add_assignment', kwargs={'workloadscenario_id': first_scen.id}), {'select_lecturer': normal_lecturer.id, 'select_module': module_2.id, 'teaching_assignment_type' : assignment_type.id, 'counted_towards_workload' : 'yes','how_many_units':'56'})
        self.client.post(reverse('eduload:add_assignment', kwargs={'workloadscenario_id': first_scen.id}), {'select_lecturer': educator_track.id, 'select_module': module_3.id, 'teaching_assignment_type' : assignment_type.id, 'counted_towards_workload' : 'yes','how_many_units':'156'})
        self.client.post(reverse('eduload:add_assignment', kwargs={'workloadscenario_id': first_scen.id}), {'select_lecturer': vice_dean.id, 'select_module': module_4.id, 'teaching_assignment_type' : assignment_type.id, 'counted_towards_workload' : 'yes','how_many_units':'356'})
        ...
        mod_code_8 = 'AS801'
        mod_code_9 = 'AS901'
        self.client.post(reverse('eduload:add_module', kwargs={'workloadscenario_id': first_scen.id}), {'module_code': mod_code_5, 'module_title' : 'module 5', 'total_hours' : '10', 'module_type' : mod_type_core.id, 'students_year_of_study' :2, 'compulsory_in_primary_programme' : False, 'compulsory_in_secondary_programme':  False, 'compulsory_in_tertiary_programme': False,'semester_offered' : Module.SPECIAL_TERM_1,   'fresh_record' : True})    
        self.client.post(reverse('eduload:add_module', kwargs={'workloadscenario_id': first_scen.id}), {'module_code': mod_code_6, 'module_title' : 'module 6', 'total_hours' : '20', 'module_type' : mod_type_elective.id, 'students_year_of_study' :2, 'compulsory_in_primary_programme' : False, 'compulsory_in_secondary_programme':  False, 'compulsory_in_tertiary_programme': False, 'semester_offered' : Module.SPECIAL_TERM_1,   'fresh_record' : True})    
        self.client.post(reverse('eduload:add_module', kwargs={'workloadscenario_id': first_scen.id}), {'module_code': mod_code_7, 'module_title' : 'module 7', 'total_hours' : '30', 'module_type' : mod_type_faculty.id,'students_year_of_study' :2, 'compulsory_in_primary_programme' : False, 'compulsory_in_secondary_programme':  False, 'compulsory_in_tertiary_programme': False, 'semester_offered' : Module.SPECIAL_TERM_2,   'fresh_record' : True})    
        self.client.post(reverse('eduload:add_module', kwargs={'workloadscenario_id': first_scen.id}), {'module_code': mod_code_8,  'module_title' : 'module 8', 'total_hours' : '40', 'module_type' : mod_type_faculty.id, 'students_year_of_study' :2, 'compulsory_in_primary_programme' : False, 'compulsory_in_secondary_programme':  False, 'compulsory_in_tertiary_programme': False,'semester_offered' : Module.UNASSIGNED,   'fresh_record' : True})        
        self.client.post(reverse('eduload:add_module', kwargs={'workloadscenario_id': first_scen.id}), {'module_code': mod_code_9,  'module_title' : 'module 9', 'total_hours' : '50', 'module_type' : mod_type_faculty.id, 'students_year_of_study' :2, 'compulsory_in_primary_programme' : False, 'compulsory_in_secondary_programme':  False, 'compulsory_in_tertiary_programme': False,'semester_offered' : Module.UNASSIGNED,   'fresh_record' : True})        
        ...
        self.client.post(reverse('eduload:add_assignment', kwargs={'workloadscenario_id': first_scen.id}), {'select_lecturer': normal_lecturer.id, 'select_module': module_5.id, 'teaching_assignment_type' : assignment_type.id, 'counted_towards_workload' : 'yes','how_many_units':'36'})
        self.client.post(reverse('eduload:add_assignment', kwargs={'workloadscenario_id': first_scen.id}), {'select_lecturer': vice_dean.id, 'select_module': module_5.id, 'teaching_assignment_type' : assignment_type.id, 'counted_towards_workload' : 'yes','how_many_units':'6'})
        self.client.post(reverse('eduload:add_assignment', kwargs={'workloadscenario_id': first_scen.id}), {'select_lecturer': normal_lecturer.id, 'select_module': module_7.id, 'teaching_assignment_type' : assignment_type.id, 'counted_towards_workload' : 'yes','how_many_units':'56'})
        self.client.post(reverse('eduload:add_assignment', kwargs={'workloadscenario_id': first_scen.id}), {'select_lecturer': educator_track.id, 'select_module': module_8.id,'teaching_assignment_type' : assignment_type.id, 'counted_towards_workload' : 'yes','how_many_units':'10'})
        self.client.post(reverse('eduload:add_assignment', kwargs={'workloadscenario_id': first_scen.id}), {'select_lecturer': vice_dean.id, 'select_module': module_8.id, 'teaching_assignment_type' : assignment_type.id, 'counted_towards_workload' : 'yes','how_many_units':'20'})
        ...
        self.client.post(reverse('eduload:add_assignment', kwargs={'workloadscenario_id': first_scen.id}), {'select_lecturer': vice_dean.id, \
        ...
        self.client.post(reverse('eduload:add_assignment', kwargs={'workloadscenario_id': first_scen.id}), {'select_lecturer': external_lecturer.id, \
                                                      'select_module': module_7.id,\
        ...
        self.client.post(reverse('eduload:manage_scenario'),{'label':new_label, 'dept' : first_dept.id, 'status': WorkloadScenario.DRAFT, 'fresh_record' : True, 'academic_year': acad_year.id})
    ...
    def testCalculateModuleTableOverYears(self):
        ...
        new_label = "new_scenario"
        self.client.post(reverse('eduload:manage_scenario'), {'label': new_label, 'dept' : test_dept.id, 'copy_from':scenario_1.id, \
    ...
    def testCalculateDepartmentProgrammeTables(self):
        ...
        track_def = EmploymentTrack.objects.create(track_name = "track default", track_adjustment = 1.0, is_adjunct = False, faculty=new_fac)
        test_svc_role = ServiceRole.objects.create(role_name = "test", role_adjustment = 2.0, faculty=new_fac)
        normal_lecturer = Lecturer.objects.create(name="normal_lecturer", fraction_appointment = 0.7, service_role = test_svc_role,employment_track=track_def, workload_scenario = wl_scen)
        ...
        vice_dean = Lecturer.objects.create(name="vice_dean", fraction_appointment = 0.5, service_role = test_svc_role, employment_track=track_def,workload_scenario = wl_scen)
        ...
        unassigned_pg_mod = Module.objects.create(module_code = "UNASSIGNED_PG", module_title = "unassigned pg sem 2", scenario_ref = wl_scen,\
                                               total_hours = 350, module_type = elective_mod, semester_offered= Module.SEM_2, \
                                               primary_programme=pg_prog)
        ...
        mod_6_pg_both_sems = Module.objects.create(module_code = "M6-PG-BOTHSEMS", module_title = "module 6 pg both sems", scenario_ref = wl_scen,\
                                               total_hours = 150, module_type = elective_mod, semester_offered= Module.BOTH_SEMESTERS , \
                                               primary_programme=pg_prog)
        ...
        mod_7_pg_and_ug = Module.objects.create(module_code = "M7-PG-AND-UG", module_title = "module 7 pg and ug", scenario_ref = wl_scen,\
                                               total_hours = 187, module_type = elective_mod, semester_offered= Module.SEM_1 , \
