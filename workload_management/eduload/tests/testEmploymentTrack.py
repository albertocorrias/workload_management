...
from eduload.models import Lecturer, WorkloadScenario, Department, EmploymentTrack, ServiceRole, UniversityStaff, Faculty,Academicyear


class TestEmploymentTrack(TenantTestCase):
    def setup_user(self):
    ...
    def test_add_remove_track(self):
        ...
        track_adj = 0.5
        self.client.post(reverse('eduload:school_page' ,  kwargs={'faculty_id': new_fac.id}),{'track_name':new_track_name,'track_adjustment' : str(track_adj), 'fresh_record': True})
        ...
        self.client.post(reverse('eduload:school_page',  kwargs={'faculty_id': new_fac.id}),{'select_track_to_remove':new_track.get().id})
    ...
    def test_edit_existing_track(self):
        ...
        track_adj = 0.5
        self.client.post(reverse('eduload:school_page',  kwargs={'faculty_id': new_fac.id}),{'track_name':track_name,'track_adjustment' : str(track_adj), 'is_adjunct' : False,'fresh_record': True})
        ...
        self.client.post(reverse('eduload:school_page',  kwargs={'faculty_id': new_fac.id}),{'track_name':track_name,\
        ...
        #Now edit the name
        new_name = "hello"
        self.client.post(reverse('eduload:school_page',  kwargs={'faculty_id': new_fac.id}),{'track_name':new_name,\
                                                                        'track_adjustment' : str(track_adj_after_change), \
                                                                         'fresh_record': False,
                                                                         'is_adjunct' : False,
                                                                         'employment_track_id': str(track_id)})
        ...
        self.client.post(reverse('eduload:school_page',  kwargs={'faculty_id': new_fac.id}),{'track_name':another_new_name,\
                                                                        'track_adjustment' : str(track_adj_after_change), \
                                                                         'fresh_record': True,
                                                                         'is_adjunct': True,
                                                                         'employment_track_id': str(track_id)})
    ...
    def test_no_duplicate_names(self):
        ...
        track_name = 'test_track'
        track_adj = 0.5
        self.client.post(reverse('eduload:school_page',  kwargs={'faculty_id': new_fac.id}),{'track_name':track_name,'track_adjustment' : str(track_adj), 'fresh_record': True})
        self.assertEqual(EmploymentTrack.objects.all().count(),1)
        self.assertEqual(EmploymentTrack.objects.filter(track_name = track_name).count(),1)

        #Try adding another one with the same name
        self.client.post(reverse('eduload:school_page',  kwargs={'faculty_id': new_fac.id}),{'track_name':track_name,'track_adjustment' : str(0.3), 'fresh_record': True})
        self.assertEqual(EmploymentTrack.objects.all().count(),1)
        self.assertEqual(EmploymentTrack.objects.filter(track_name = track_name).count(),1)

        #Now we ligitinamtely add another one
        new_name = "hello"
        self.client.post(reverse('eduload:school_page',  kwargs={'faculty_id': new_fac.id}),{'track_name':new_name,'track_adjustment' : str(0.75), 'fresh_record': True})
    ...
    def test_edit_lecturer_change_track(self):
        ...
        track_adj_2 = 9
        self.client.post(reverse('eduload:school_page',  kwargs={'faculty_id': new_fac.id}),{'track_name':new_track_name_2,'track_adjustment' : str(track_adj_2), 'fresh_record': True})
    ...
    def test_add_remove_track_with_lecturer_multiple_scen(self):
        ...
        track_adj = 0.5
        self.client.post(reverse('eduload:school_page',  kwargs={'faculty_id': new_fac.id}),{'track_name':new_track_name,'track_adjustment' : str(track_adj),'fresh_record': True})
        self.client.post(reverse('eduload:school_page',  kwargs={'faculty_id': new_fac.id}),{'track_name':new_track_name_2,'track_adjustment' : str(track_adj), 'fresh_record': True})
        new_track = EmploymentTrack.objects.filter(track_name = new_track_name)
        new_track_2 = EmploymentTrack.objects.filter(track_name = new_track_name_2)
        ...
        self.client.post(reverse('eduload:add_professor',  kwargs={'workloadscenario_id': new_scen.id}),{'name':'ted','fraction_appointment' : '0.5',    \
                                                                'workload_scenario': new_scen.id, 'employment_track' : new_track.get().id, \
                                                                'service_role' : def_role.id, 'is_external': False,'fresh_record' : True})
        
        #and the same lecturers to another scenario (one changed track)
        self.client.post(reverse('eduload:add_professor',  kwargs={'workloadscenario_id': new_scen_2.id}),{'name':'bob','fraction_appointment' : '0.5',    \
                                                                'workload_scenario': new_scen_2.id, 'employment_track' : new_track.get().id, \
                                                                'service_role' : def_role.id,'is_external': False, 'fresh_record' : True})
        self.client.post(reverse('eduload:add_professor',  kwargs={'workloadscenario_id': new_scen_2.id}),{'name':'ted','fraction_appointment' : '0.5',    \
