...
from eduload.models import Faculty, Department, Module, ModuleType, WorkloadScenario,UniversityStaff, TeachingAssignmentType,\
...
class TestUserPermissions(TenantTestCase):
    ...
    def testSuperUserPageAccess(self):
        ...
        #module pages  access
        response = self.client.get(reverse('eduload:module',  kwargs={'module_code': mod_code}))
        self.assertEqual(response.status_code, 200) #no issues
        self.assertEqual(('error_message' in response.context), False)
        self.assertEqual(response.context["module_title"], "First module")
        self.assertEqual(response.context["module_code"], mod_code)
        response = self.client.get(reverse('eduload:module',  kwargs={'module_code': mod_code+"_2"}))
        self.assertEqual(response.status_code, 200) #no issues
        ...
        response = self.client.get(reverse('eduload:department', kwargs={'department_id': new_dept.id}))
        self.assertEqual(response.status_code, 200) #No issues
        self.assertEqual(('error_message' in response.context), False)
        self.assertEqual(response.context["dept_name"], "test_dept")
        #Accreditation page  access
        response = self.client.get(reverse('eduload:accreditation', kwargs={'programme_id': new_prog.id}))
        self.assertEqual(response.status_code, 200) #No issues
        self.assertEqual(('error_message' in response.context), False)
        self.assertEqual(response.context["programme_name"], "new_prog")
        #accreditation report page access
        response = self.client.get(reverse('eduload:accreditation_report', kwargs={'programme_id': new_prog.id, 'start_year' : 2020, 'end_year':2021,'compulsory_only':1}))
        self.assertEqual(response.status_code, 200) #No issues
        self.assertEqual(('error_message' in response.context), False)
        self.assertEqual(response.context["programme_name"], "new_prog")
        #Lecturer page access 
        response = self.client.get(reverse('eduload:lecturer_page', kwargs={'lecturer_id': lecturer_1.id}))
        self.assertEqual(response.status_code, 200) #No issues
        self.assertEqual(('error_message' in response.context), False)
        self.assertEqual(response.context["lec_name"], "lecturer_1")
        #Survey results page access
        response = self.client.get(reverse('eduload:survey_results', kwargs={'survey_id': slo_survey_1.id}))
        self.assertEqual(response.status_code, 200) #No issues
        self.assertEqual(('error_message' in response.context), False)
        self.assertEqual(('survey_details' in response.context), True)
        #Programme survey input page access
        response = self.client.get(reverse('eduload:input_programme_survey_results', kwargs={'programme_id': new_prog.id, 'survey_id': slo_survey_1.id}))
        self.assertEqual(response.status_code, 200) #No issues
        self.assertEqual(('error_message' in response.context), False)
        self.assertEqual(response.context["programme_id"], new_prog.id)
        #Module survey input page access
        response = self.client.get(reverse('eduload:input_module_survey_results', kwargs={'module_code': mod_code, 'survey_id': mlo_survey_1.id}))
        self.assertEqual(response.status_code, 200) #No issues
        self.assertEqual(('error_message' in response.context), False)
        self.assertEqual(response.context["module_code"], mod_code)
        ...
        #Faculty  page access
        response = self.client.get(reverse('eduload:school_page',  kwargs={'faculty_id': new_fac.id}))
        self.assertEqual(response.status_code, 200) #No issues
        self.assertEqual(('error_message' in response.context), False)
        self.assertEqual(response.context["school_name"],new_fac.faculty_name)
        #Try with another faculty - SHOULD BE NO ACCESS
        response = self.client.get(reverse('eduload:school_page',  kwargs={'faculty_id': new_fac_2.id}))
        self.assertEqual(('error_message' in response.context), True)
        ...
        response = self.client.get(reverse('eduload:department', kwargs={'department_id': new_dept_2.id}))
        self.assertEqual(response.status_code, 200) #No issues
        self.assertEqual(('error_message' in response.context), True)
        ...
        response = self.client.get(reverse('eduload:accreditation', kwargs={'programme_id': new_prog_2.id}))
        self.assertEqual(response.status_code, 200) #No issues
        self.assertEqual(('error_message' in response.context), True)       
        ...
        response = self.client.get(reverse('eduload:accreditation_report', kwargs={'programme_id': new_prog_2.id, 'start_year' : 2020, 'end_year':2021,'compulsory_only':1}))
        self.assertEqual(('error_message' in response.context), True)   
        ...
        response = self.client.get(reverse('eduload:lecturer_page', kwargs={'lecturer_id': lecturer_2.id}))
        self.assertEqual(response.status_code, 200) #No issues
        ...
        response = self.client.get(reverse('eduload:survey_results', kwargs={'survey_id': slo_survey_2.id}))
        self.assertEqual(response.status_code, 200) #No issues
        self.assertEqual(('error_message' in response.context), True)
        ...
        response = self.client.get(reverse('eduload:input_programme_survey_results', kwargs={'programme_id': new_prog_2.id, 'survey_id': slo_survey_2.id}))
        self.assertEqual(response.status_code, 200) #No issues
        self.assertEqual(('error_message' in response.context), True)
        ...
        response = self.client.get(reverse('eduload:input_module_survey_results', kwargs={'module_code': mod_code+"_2", 'survey_id': mlo_survey_2.id}))
        self.assertEqual(response.status_code, 200) #No issues
        self.assertEqual(('error_message' in response.context), True)
        ...
        response = self.client.get(reverse('eduload:survey_results', kwargs={'survey_id': mlo_survey_1.id}))
        self.assertEqual(response.status_code, 200) #No issues
        self.assertEqual(('error_message' in response.context), False)
