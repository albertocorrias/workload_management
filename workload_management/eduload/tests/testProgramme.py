...
class TestProgramme(TenantTestCase):
    ...
    def test_add_remove_programme(self):
        ...
        self.client.post(reverse('eduload:manage_programme_offered', kwargs={'dept_id': dept.id}),{'programme_name':new_programme_name, 'primary_dept': dept.id,'fresh_record':True})
        ...
        response = self.client.get(reverse('eduload:department', kwargs={'department_id': dept.id}))
        self.assertEqual(response.status_code, 200) #No issues

        #Add another one
        another_programme_name = 'Second Masters'
        self.client.post(reverse('eduload:manage_programme_offered', kwargs={'dept_id': dept.id}),{'programme_name':another_programme_name, 'primary_dept': dept.id,'fresh_record':True})
        ...
        self.client.post(reverse('eduload:manage_programme_offered', kwargs={'dept_id': dept.id}),{'programme_name':new_programme_name, 'primary_dept': dept.id,'prog_id' : first_prog.id,'fresh_record':False})
        ...
        self.client.post(reverse('eduload:manage_programme_offered', kwargs={'dept_id': dept.id}),{'programme_name':new_valid_name, 'primary_dept': dept.id,'prog_id' : first_prog.id,'fresh_record':False})
        self.assertEqual(ProgrammeOffered.objects.all().count(),2) #Two offered
        ...
        self.client.post(reverse('eduload:remove_programme_offered',kwargs={'dept_id': dept.id}),{'select_programme_to_remove':second_prog.id})
    ...
    def test_add_remove_programme_with_modules(self):
        ...
        seondary_name = "UG"
        self.client.post(reverse('eduload:manage_programme_offered', kwargs={'dept_id': dept.id}),{'programme_name':seondary_name, 'primary_dept': dept.id,'fresh_record':True})
        ...
        self.client.post(reverse('eduload:remove_programme_offered',kwargs={'dept_id': dept.id}),{'select_programme_to_remove':new_prog.id})
        ...
        self.client.post(reverse('eduload:remove_programme_offered',kwargs={'dept_id': dept.id}),{'select_programme_to_remove':secondary_prog_obj.id})
        self.assertEqual(Module.objects.filter(primary_programme__programme_name=new_programme_name).count(),0)
        self.assertEqual(Module.objects.filter(module_code=mod_code).count(),1)#Module still there
        self.assertEqual(Module.objects.filter(primary_programme__isnull=True).count(),1)#but with NULL reference (SET_NULL in foreign key was set)
    ...
    def test_add_remove_subprogramme(self):
        ...
        self.client.post(reverse('eduload:manage_subprogramme_offered', kwargs={'dept_id': dept.id}),{'sub_programme_name':new_subprogramme_name, 'main_programme': new_prog.id,'fresh_record':True})
        ...
        self.client.post(reverse('eduload:remove_subprogramme_offered', kwargs={'dept_id': dept.id}),{'select_subprogramme_to_remove' : sub_prog.id})
        ...
        self.client.post(reverse('eduload:manage_subprogramme_offered', kwargs={'dept_id': dept.id}),{'sub_programme_name':new_name, 'sub_prog_id': sub_prog_object.id, 'main_programme': new_prog.id,'fresh_record':False})
        self.assertEqual(SubProgrammeOffered.objects.all().count(),1)
        self.assertEqual(SubProgrammeOffered.objects.filter(sub_programme_name = new_subprogramme_name).count(),0)
    ...
    def test_add_remove_programme_with_modules(self):
        ...
        #Add one programme
        new_programme_name = 'Masters'
        self.client.post(reverse('eduload:manage_programme_offered', kwargs={'dept_id': new_dept.id}),{'programme_name':new_programme_name, 'primary_dept': new_dept.id,'fresh_record':True})
        new_prog = ProgrammeOffered.objects.filter(programme_name=new_programme_name).get()
        ...
        #Now add a sub-programme to it
        new_subprogramme_name = "special_sub"
        self.client.post(reverse('eduload:manage_subprogramme_offered', kwargs={'dept_id': new_dept.id}),{'sub_programme_name':new_subprogramme_name, 'main_programme': new_prog.id,'fresh_record':True})
        self.assertEqual(SubProgrammeOffered.objects.all().count(),1)
        self.assertEqual(SubProgrammeOffered.objects.filter(sub_programme_name = new_subprogramme_name).count(),1)
        self.assertEqual(SubProgrammeOffered.objects.filter(main_programme__id = new_prog.id).count(),1)
        ...
        self.client.post(reverse('eduload:manage_subprogramme_offered', kwargs={'dept_id': new_dept.id}),{'sub_programme_name':secondary_subprogramme_name, 'main_programme': new_prog.id,'fresh_record':True})
