...
def scenario_view(request, workloadscenario_id):
    ...
    template = loader.get_template('eduload/workload.html')
    context = {
...
def school_page(request,faculty_id):
    ...
    if request.method =='POST':
        ...
        #trigger a GET
        return HttpResponseRedirect(reverse('eduload:school_page',  kwargs={'faculty_id': faculty_id}))
    else: #This is a get
        ...
        template = loader.get_template('eduload/school_page.html')
...
def workloads_index(request):
    #populate_database(schema='nusdemo')
    menus = CheckUserInput(request) 
    ...
    template = loader.get_template('eduload/workloads_index.html')
...
def lecturer_page(request,lecturer_id):
    ...
    template = loader.get_template('eduload/lecturer_page.html')
    context = {
        'lec_name' : lec_name,
...
def faculty_report(request):
    ...
    template = loader.get_template('eduload/faculty_report.html')
    context = {
...
def department(request,department_id):
    ...
    if request.method =='POST':
        ...
        #Trigger a GET
        return HttpResponseRedirect(reverse('eduload:department',  kwargs={'department_id': department_id}))
        ...
        template = loader.get_template('eduload/department.html')
        context = {
...
def module(request, module_code):
    ...
    if request.method =='POST':
        ...
        return HttpResponseRedirect(reverse('eduload:module', kwargs={'module_code' : module_code}));#Trigger a get
    else:#This is a get
        ...
        template = loader.get_template('eduload/module.html')
        context = {
...
def accreditation(request,programme_id):
    ...
    if request.method =='POST':
        ...
        if select_report_years_form.is_valid():
            ...
            return HttpResponseRedirect(reverse('eduload:accreditation_report', 
        ...
        return HttpResponseRedirect(reverse('eduload:accreditation', kwargs={'programme_id' : programme_id}));#Trigger a get
        ...
        template = loader.get_template('eduload/accreditation.html')
        context = {
...
def accreditation_report(request,programme_id, start_year,end_year,compulsory_only):
    ...
    if request.method == 'GET':
        ...
        template = loader.get_template('eduload/accreditation_report.html')
        context = {
            'programme_id' : programme_id,
...
def input_module_survey_results(request,module_code,survey_id):
    ...
    survey_qs  =Survey.objects.select_related('likert_labels').filter(id = survey_id)
    if (survey_qs.count() != 1):
        #This should really never happen, but just in case the user enters some random number...
        template = loader.get_template('eduload/errors_page.html')
        context = {
    ...
    if request.method =='POST':
        ...
        template = loader.get_template('eduload/module_survey_input.html')
    ...
    return HttpResponseRedirect(reverse('eduload:module',  kwargs={'module_code': module_code}))
...
def input_programme_survey_results(request,programme_id,survey_id):
    ...
    if request.method =='POST':
        ...
        return HttpResponseRedirect(reverse('eduload:accreditation',  kwargs={'programme_id': programme_id}))
        ...
        template = loader.get_template('eduload/survey_input.html')
        context = {
            'form_to_show' : form_to_show,
            'programme_id' : programme_id,
            'survey_id' :survey_id,
...
def survey_results(request,survey_id):
    ...
    if request.method=="GET":
        ...
        all_survey_details = CalculateSurveyDetails(survey_id)
        template = loader.get_template('eduload/survey_results.html')
...
def add_assignment(request,workloadscenario_id):
    ...
    return HttpResponseRedirect(reverse('eduload:scenario_view',  kwargs={'workloadscenario_id': workloadscenario_id}))
...
def edit_module_assignments(request, module_id):
    ...
    #Otherwise do nothing
    return HttpResponseRedirect(reverse('eduload:scenario_view',  kwargs={'workloadscenario_id': scenario_id}))
...
def manage_scenario(request):
    ...
    return HttpResponseRedirect(reverse('eduload:workloads_index'))
...
def add_professor(request, workloadscenario_id):
    ...
    return HttpResponseRedirect(reverse('eduload:scenario_view',  kwargs={'workloadscenario_id': selected_scen.id}))
...
def remove_professor(request,workloadscenario_id):
    if request.method =='POST':
        ...
        if (form.is_valid()):
            ...
            #else, nothing, the lecturer remains on the list, but we re-generate the menu, just in case
            #Force re-population of user menu
            user_qs = UniversityStaff.objects.select_related("department","faculty","lecturer","user").prefetch_related("user__groups").filter(user__id = request.user.id)
            new_menu = DetermineUserMenu(user_qs.get(), is_super_user=request.user.is_superuser,force_population=True)
...
def add_module(request,workloadscenario_id):
    ...
    if "HTTP_REFERER" in request.META:
        if ("department" in request.META["HTTP_REFERER"]):#The edit module form appears in two pages, the workload page and the "department" page
            ...
            return HttpResponseRedirect(reverse('eduload:department',  kwargs={'department_id': dept_id}))
        else:
            return HttpResponseRedirect(reverse('eduload:scenario_view',  kwargs={'workloadscenario_id': workloadscenario_id}))
    else:
        return HttpResponseRedirect(reverse('eduload:scenario_view',  kwargs={'workloadscenario_id': workloadscenario_id}))
